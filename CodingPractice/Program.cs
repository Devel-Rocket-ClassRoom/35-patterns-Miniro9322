...
{
    Console.WriteLine($"{person.Name}({person.Age}): {DescribePerson(person)}");
}
...
{
    Student{ School: var school, Name: var name, Age: var age } => $"{name}: {age}세 학생, {school} 재학",
    Employee { Company: var company, Name: var name, Age: var age } => $"{name}: {age}세 직장인, {company} 근무",
    { Name:var name, Age: var age } => $"{name}: {age}세 직장인",
    _ => "정보 없음"
};
...
string GetTemperatureDescription(string season, bool isDaytime) =>
    (season, isDaytime) switch
    {
        ("봄", true) => "따뜻한 낮",
        ("봄", false) => "선선한 밤",
        ("여름", true) => "무더운 낮",
...
Console.WriteLine($"4: {DescribeNumber(4)}");
Console.WriteLine($"7: {DescribeNumber(7)}");
Console.WriteLine($"-6: {DescribeNumber(-6)}");
Console.WriteLine($"-3: {DescribeNumber(-3)}");
